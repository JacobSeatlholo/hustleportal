export default async function handler(req, res) {
  ...
  if (body?.event === 'validate_code') {
    ...
    if (envCodes.includes(code)) {
      // Log the redemption to Sheet (fire and forget)
      const SHEET_URL = process.env.SHEET_URL ||
        'https://script.google.com/macros/s/AKfycbxg-NPt-XxYWuoSLnq7R7XlNVU4kJnBiAeyGGB1gOoocvzErwRPO2iOwWnyIni29V1wBg/exec';
      fetch(SHEET_URL, {
        method: 'POST',
        ...
        body: JSON.stringify({
          event: 'leads',
          data: {
            name: body?.data?.business || 'Unknown',
            email: '',
            source: `Pro Code Redeemed (recurring) — ${code}`,
          },
        }),
      }).catch(() => {});
